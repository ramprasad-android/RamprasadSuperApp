package `in`.rramprasad.superapp.features.logs
...
fun LogsScreen(
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(paddingValues).padding(16.dp)) {
            Text(text = "Logs Screen", fontSize = 24.sp)
        }
    }
}

class LogsNavEntryProvider(
    private val onScreenSelected: (Destination) -> Unit
) : NavEntryProvider {
    override fun provideEntry(key: Destination): NavEntry<out Destination>? {
        return if (key is Destination.Logs) {
            NavEntry(key) {
                LogsScreen(onScreenSelected = onScreenSelected)
            }
        } else null
    }
}
