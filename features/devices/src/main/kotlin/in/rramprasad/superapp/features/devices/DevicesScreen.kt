package `in`.rramprasad.superapp.features.devices
...
fun DevicesScreen(
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(paddingValues).padding(16.dp)) {
            Text(text = "Devices Screen", fontSize = 24.sp)
            Spacer(modifier = Modifier.height(16.dp))
            Button(onClick = { onScreenSelected(Destination.Home) }) {
                Text("Back to Home")
            }
        }
    }
...
class DevicesNavEntryProvider(
    private val onScreenSelected: (Destination) -> Unit
) : NavEntryProvider {
    override fun provideEntry(key: Destination): NavEntry<out Destination>? {
        return if (key is Destination.Devices) {
            NavEntry(key) {
                DevicesScreen(onScreenSelected = onScreenSelected)
            }
        } else null
    }
}
